...
export const ProfileCard: React.FC<ProfileCardProps> = ({ profile }) => {
  ...
  return (
    <Link
      ...
      className="group relative bg-[#270E15] rounded-xl overflow-hidden border border-ouro/30 hover:border-ouro shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1.5 flex flex-col"
    >
      {/* Container de Imagem */}
      <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#18080C]">
        ...
        {/* Badges Flutuantes: Online & Categoria */}
        <div className="absolute top-3 right-3 flex flex-col items-end space-y-1.5 z-10">
          {profile.isOnline && (
            <div className="flex items-center space-x-1.5 px-2.5 py-1 rounded-md bg-black/70 border border-emerald-500/40 text-emerald-400 text-[11px] font-bold backdrop-blur-md">
      ...
      <div className="p-4 sm:p-5 flex-1 flex flex-col justify-between space-y-3 bg-[#270E15]">
        ...
        <div className="flex items-center justify-between text-[11px] text-gray-300 pt-2.5 border-t border-white/10">
          <span>Alt: <strong className="text-white font-bold">{profile.specs.height}</strong></span>
          <span>Peso: <strong className="text-white font-bold">{profile.specs.weight}</strong></span>
          <span>Cabelo: <strong className="text-white font-bold">{profile.specs.hair}</strong></span>
        </div>
