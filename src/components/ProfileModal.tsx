...
export const ProfileModal: React.FC<ProfileModalProps> = ({ profile, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/90 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-200">
      ...
      {/* Modal Card Box */}
      <div className="relative w-full max-w-4xl bg-grafite rounded-none sm:rounded-none border border-white/10 shadow-2xl overflow-hidden max-h-[92vh] sm:max-h-[90vh] flex flex-col my-0 sm:my-auto">
        
        {/* Top Header Bar */}
        <div className="sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 py-3 bg-onix/95 border-b border-white/10 backdrop-blur-md">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-campo bg-white/10 hover:bg-white/20 text-marfim transition-colors"
              aria-label="Fechar"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 pb-28 sm:pb-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
            ...
            <div className="md:col-span-6 space-y-3">
              {/* Main Photo View */}
              <div className="relative aspect-[3/4] w-full rounded-none overflow-hidden bg-grafite border border-white/10 shadow-lg">
                <img
            ...
            <div className="md:col-span-6 space-y-5">
              ...
              <div className="border-b border-white/10 pb-4 space-y-2">
                <div className="flex items-center justify-between">
                  <h2 className="text-2xl sm:text-3xl font-display font-normal text-marfim tracking-tight flex items-center space-x-2">
                ...
                <div className="pt-2 flex items-center justify-between">
                  ...
                  <div className="text-xl sm:text-2xl font-black text-ouro">
                    {profile.hourlyRate}
                  </div>
                </div>
              </div>
              ...
              {/* Tagline & Bio */}
              <div className="space-y-2 bg-grafite p-4 rounded-none border border-white/10">
                <h4 className="text-xs font-bold text-ouro uppercase tracking-wider flex items-center space-x-1">
                  <Sparkles className="w-3.5 h-3.5" />
                  <span>Sobre mim</span>
                </h4>
                <p className="text-xs sm:text-sm text-marfim leading-relaxed italic">
                  "{profile.bio}"
                </p>
              </div>
              ...
              <div className="hidden md:block pt-2">
                <a
                  ...
                  className="w-full flex items-center justify-center space-x-3 py-4 rounded-none bg-whatsapp hover:bg-whatsapp-hover text-marfim font-extrabold text-lg shadow-xl transition-all transform hover:scale-[1.02]"
                >
          ...
          <div className="pt-4 border-t border-white/10 space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {profile.services.map((service, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1.5 rounded-campo bg-verificado/10 border border-verificado/20 text-verificado-texto text-xs font-semibold flex items-center space-x-1"
                >
                  <span>✓</span>
                  <span>{service}</span>
                </span>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {profile.locations.map((loc, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1.5 rounded-campo bg-white/5 border border-white/10 text-marfim text-xs font-medium"
                >
                  {loc}
                </span>
              ))}
          ...
          <div className="p-3.5 rounded-none bg-ouro/10 border border-ouro/20 text-champanhe text-xs flex items-center space-x-2">
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 p-3 bg-onix/95 border-t border-white/15 backdrop-blur-xl shadow-2xl">
          <a
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="w-full flex items-center justify-center space-x-3 py-3.5 rounded-none bg-whatsapp hover:bg-whatsapp-hover text-marfim font-extrabold text-base shadow-2xl active:scale-95 transition-all"
          >
