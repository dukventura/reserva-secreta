...
export const AdvertiseModal: React.FC<AdvertiseModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
      <div className="relative w-full max-w-lg bg-grafite rounded-none border border-ouro/30 shadow-2xl p-6 sm:p-8 space-y-6">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 rounded-campo bg-white/10 hover:bg-white/20 text-marfim transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="text-center space-y-2">
          <div className="w-12 h-12 rounded-none bg-ouro/10 border border-ouro/30 flex items-center justify-center mx-auto text-ouro">
            <Crown className="w-6 h-6" />
          </div>
          <h2 className="text-2xl font-display font-normal text-marfim">
        ...
        {/* Benefits list */}
        <div className="space-y-2 bg-white/5 p-3.5 rounded-none border border-white/10 text-xs text-nevoa">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className="w-full bg-grafite text-marfim text-sm rounded-campo px-3 py-2.5 border border-white/15 focus:border-ouro outline-none"
              >
            ...
            <div>
              <label className="block text-xs font-semibold text-nevoa mb-1">Categoria Pretendida</label>
              <select
          ...
          <div>
            <label className="block text-xs font-semibold text-nevoa mb-1">Seu WhatsApp de Atendimento</label>
            <input
              type="tel"
              required
          ...
          <button
            type="submit"
            className="w-full flex items-center justify-center space-x-2 py-3.5 rounded-campo bg-whatsapp hover:bg-whatsapp-hover text-marfim font-extrabold text-sm shadow-xl transition-all"
          >
